...
public class Minesweeper {
    public char[][] updateBoard(char[][] board, int[] click) {
        ...
        q.offer(click);
        board[click[0]][click[1]] = 'Q';//mark in queue already
        while (!q.isEmpty()) {
            //System.out.println(q.size());
            int[] cur = q.poll();
            int adjantMineNumber = 0;
            List<int[]> adjantNodeList = new ArrayList<>();
            for (int i=-1;i<2;i++) {
                for (int j=-1;j<2;j++) {
                    if (i==0 && j==0) {
                        continue;
                    ...
                    if (row>=0 && row<board.length && col>=0 && col<board[0].length) {
                        if (board[row][col]=='E') {
                            adjantNodeList.add(new int[]{row,col});
                        }
                        if (board[row][col]=='M') {
                            adjantMineNumber++;
                        }
            ...
            if (adjantMineNumber>0) {
                board[cur[0]][cur[1]] = (char)('0'+adjantMineNumber);
            } else {
                board[cur[0]][cur[1]] = 'B';
                for (int[] each:adjantNodeList) {
                    q.offer(each);
                    board[each[0]][each[1]] = 'Q';
                }
            }
        }
        return board;
