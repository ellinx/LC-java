...
public class CanIWin {
	public boolean canIWin(int maxChoosableInteger, int desiredTotal) {
        ...
        int used = 0;//if bit i is one it means i is used.
        return dfs(map, used, maxChoosableInteger, desiredTotal);
    }
    
    private boolean dfs(Map<Integer, Boolean> map, int used, int maxChoosableInteger, int target) {
        if (target<=0)
            return false;
        if (!map.containsKey(used)) {
            for (int i=1;i<=maxChoosableInteger;i++) {
                if ((used&(1<<i))==0) {
                    //toggle as one win the other lose
                    if (!dfs(map, used|(1<<i), maxChoosableInteger, target-i)) {
                        map.put(used, true);
                        break;
                    }
                }
